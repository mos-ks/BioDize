...
_PROMPT = (
  ...
  "CRITICAL for checkboxes: an option is 'selected' ONLY when its box carries a VISIBLE mark "
  "(✓, ✗, x, a filled/crossed box). If the box is EMPTY, leave `selected` empty and set "
  "is_blank=true — NEVER infer or default a confirmation box to 'Ja'/checked when nothing is "
  "actually marked; a blank required checkbox is a real finding, not something to fill in. "
  ...
  "CRITICAL for calculation rows written like 'Parameter = A / 1,31 kg/L × B = result': the "
  "PRINTED numbers that belong to the formula itself (a fixed density such as '1,31 kg/L' or "
  "'1,81 kg/L', and the operators) are NOT field values — NEVER report a printed formula constant "
  "as a parameter's value (e.g. do NOT read the density '1,31' as the 'c … [g/L]' concentration). "
  "Each parameter's value is the HANDWRITTEN number in ITS OWN cell, and the result is the "
  "handwritten number on the final '=' line; read the cells in place, do not shift them up or down. "
